...
def gauss(x,a,m,s):
    return a*Num.e**(-(x-m)**2/(2*s**2))


pfdfile = raw_input("pfd file?\n")
pfdfile=pfd(pfdfile)
dm = float(sproc.Popen(["grep DM *par | head -1 | awk '{print $2}'"],stdout=sproc.PIPE,shell=True).communicate()[0])
(y,x)=pfdfile.plot_chi2_vs_DM(dm-0.5*dm,dm+0.5*dm)
param,cov=curve_fit(gauss,x[y>y.max()-0.051*y.max()],y[y>y.max()-0.05*y.max()],p0=[y.max(),x[y==y.max()],0.2*y.max()])
print "A = %s, mu=%s, sigma=%s" %(param[0],param[1],param[2])
g=[]
for i in range(len(x)):
    	g.append(gauss(x[i],param[0],param[1],param[2]))

tmp=open('pfd_chi2.txt','w')
for i,j,k in zip(x,y,g):
    tmp.write("%s %s %s\n" %(i,j,k))
tmp.close()
